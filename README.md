...
| 3 | Poster images break if movie title doesn't match drawable filename | 🟡 Medium |
| 4 | No client-side input validation (password strength, rating range) | 🟡 Medium |
| 5 | No offline/network error handling or retry logic | 🟡 Medium |
| 6 | `SignUpActivity` creates its own `RequestQueue` instead of using `VolleySingleton` | 🟢 Low |
| 7 | No automated unit or integration tests | 🟢 Low |
